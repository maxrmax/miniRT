...
/*   Updated: 2025/12/08 21:25:48 by mring            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "miniRT.h"

void	check_cap(t_obj *obj, t_cap *cap, t_vec3 cap_center)
{
	t_vec3	to_center;
...
void	cap_calc(t_obj *obj)
{
	...
	{
		check_cap(obj, &bottom, obj->data.cy.center);
		top_center = vec_add(obj->data.cy.center, vec_mult(obj->data.cy.axis,
					obj->data.cy.height));
		check_cap(obj, &top, top_center);
	}
}
...
// TODO: split cy_base for decluttering of struct.
// calc_ray_perp(), calc_oc_perp(), calc_disc()
double	cy_base(t_obj *obj)
{
	...
	obj->data.cy.oc_perp = vec_sub(obj->data.cy.objcent,
			vec_mult(obj->data.cy.axis, vec_dot(obj->data.cy.objcent,
					obj->data.cy.axis)));
	a = vec_dot(obj->data.cy.ray_perp, obj->data.cy.ray_perp);
	b = 2.0 * vec_dot(obj->data.cy.ray_perp, obj->data.cy.oc_perp);
	c = vec_dot(obj->data.cy.oc_perp, obj->data.cy.oc_perp)
